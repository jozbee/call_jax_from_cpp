...
| `output index <i> is out of range: function '<name>' has <n> outputs` | `std::out_of_range`, `output`/`output_raw`/`output_spec` | The twin of the input message above, for a result index past the end. | Same fix: `num_outputs()` and `find_output(name)`. **Debug mode only** for the accessors; `output_spec` checks in every build. |
| `<path>: schema 1 sidecar has no <inputs\|outputs> sizes` | `LoadError`, sidecar | A version 1 sidecar without `args_info.sizes` or `out_info.sizes`. Usually a hand-edited file, or a version 2 sidecar whose `schema` key was removed. | Re-export. Version 2 is what the current exporter writes; version 1 is read only for artifacts predating it. |
| `<path>: schema 1 dtype '<name>' is not one of <list>` | `LoadError`, sidecar | A version 1 sidecar naming a dtype this runtime does not carry. | Re-export; the message lists what is supported. |
| `<path>: schema 1 size <n> is negative` | `LoadError`, sidecar | A negative extent in a version 1 size list. The file is corrupt. | Re-export. |
| `<path>: <inputs\|outputs> <i> is not an object` | `LoadError`, sidecar | A version 2 `inputs` or `outputs` entry that is not a JSON object, so it cannot describe an array. | Re-export; do not hand-edit a sidecar, whose checksums also have to match. |
| `<path>: cannot read the artifact names: <detail>` | `LoadError`, sidecar | The `artifacts` block is missing or malformed, so the loader cannot tell which files to open. | Re-export. If the sidecar was written by a newer jax2exec, check `schema` against what this build supports. |
