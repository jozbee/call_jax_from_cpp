...
make plugin          # the prebuilt PJRT CPU plugin, sha256-verified
                     ...
                     #  or docker: see below)
make                 # the library and examples 01-04 (05 builds with colcon)
...
- **Prebuilt plugins.** There is no official prebuilt CPU PJRT C-API plugin, so
  ...
  checksums one, `make plugin-source` builds it from the fork. JAX 0.11.0 has
  a `linux-x86_64` asset; *`linux-aarch64` is not published*, so there
...
program, built with colcon and hosted by `controller_manager`. Nonlinear
model-predictive control is the motivating application, and the reason a late
answer counts as a wrong one — but **no MPC code ships here**.
