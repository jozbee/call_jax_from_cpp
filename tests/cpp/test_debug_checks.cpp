 ...
 *        noticed.
 *
 * `FunctionOptions::debug` and `FunctionOptions::check_values` turn a silent
 * corruption into an exception, and the argument for them is what happens
 * when they are off.  So each scenario runs twice, with `--debug` and
 * without, and the pair is the test:
 *
 ...
 *
 * An artifact that cannot carry its scenario -- no floating-point input to
 * make non-finite, no bool input -- is exit 1 saying so, never a quiet pass.
 *
 * Output is one line: `THREW: <what()>` or `NO-THROW`.  The scenarios whose
 * damage lands in an output arena append ` nan_in_output=0|1` to either line,
 * set by any non-finite value in any floating-point output; on the `THREW:`
 * line it backs the claim that the check fired *and* the outputs are clean.
 * `reentrant` appends ` attempts=<n>` instead: a collision cannot be forced,
 * so it reports how many times it tried and lets pytest decide.
 *
 * Exit 0 whether the mistake was caught or not -- both are results.  Exit 1 is
 * for a bad command line, an artifact that cannot carry the scenario, and a
 * failure that is not the one being provoked.
 */
#include <atomic>
#include <chrono>
#include <cmath>
...
/// Past the end of any signature this project exports, and the same index
/// `example_01_basic --debug` uses, so the two messages match.
constexpr std::size_t kWildIndex = 99;

/// Losing the race is normal: the intruder can land between two of the
/// holder's calls.  Enough tries for the guard to fire many times over, few
/// enough that a build where it never fires still ends.
constexpr int kMaxAttempts = 32;

/// So a wedged holder thread ends the test instead of the test run.
constexpr std::chrono::seconds kRaceDeadline{5};

/// A failure that is not the one being provoked: the measurement did not
/// happen.  Reaches stderr and exit 1, where the scenario's own exception
/// reaches stdout as `THREW:` and exit 0.
...
constexpr struct {
  ...
  Scenario scenario;
    ...
    {"oob-input", Scenario::OobInput},
    {"oob-output", Scenario::OobOutput},
    {"dtype-mismatch", Scenario::DtypeMismatch},
    {"nonfinite-input", Scenario::NonfiniteInput},
    {"nonfinite-output", Scenario::NonfiniteOutput},
    {"bool-value", Scenario::BoolValue},
    {"reentrant", Scenario::Reentrant},
};

std::optional<Scenario> parse_scenario(const std::string& text) {
  for (const auto& known : kScenarios) {
    if (text == known.name) {
...
/// Where the deliberately wrong pointers go.  Volatile because with `debug`
/// off the accessors are a load and a cast, and a discarded result is a
/// discarded test.
...
/// `01_basic`'s `fun(A, b) -> (x, r)`, at whatever size it was exported.
...
void fill_basic(pjrt::Function& f, bool singular) {
  ...
  for (std::size_t i = 0; i < n; ++i) {
    for (std::size_t j = 0; j < n; ++j) {
      // Diagonally dominant, hence well conditioned: a non-finite output is a
      // bug here, not the matrix's doing.
...
/// Put a plausible problem in the input arenas before the mistake is made.
/// The zeroed arenas the loader leaves are callable, but a solver handed
/// all-zero plant parameters can produce a non-finite result on its own,
/// indistinguishable from the corruption the scenario is about.
void fill_inputs(pjrt::Function& f, bool singular) {
  ...
  try {
    ...
    cjfc::workload::write_reference(f.input<double>(cjfc::workload::kInXRef),
                                    dims, 0);
  } catch (const std::runtime_error&) {
    // Neither known signature: the zeroed arenas already survived warm-up.
  }
}

/// Call once on good inputs, so the output arenas hold a clean result before
/// the mistake is made.  Without this `nan_in_output` reports the warm-up:
/// `basic` warms up on zeroed arenas, and a zero matrix inverts to nan.
...
template <class T>
bool any_nonfinite(const void* arena, std::size_t numel) {
  const T* values = static_cast<const T*>(arena);
...
/// Integer and bool arenas have no invalid bit patterns and are skipped, as
/// the library's own `check_values` audit skips them.
bool nonfinite_in_outputs(const pjrt::Function& f) {
  for (std::size_t i = 0; i < f.num_outputs(); ++i) {
    ...
    if (f.output_dtype(i) == pjrt::DType::Float64 &&
        any_nonfinite<double>(arena, numel)) {
      return true;
    }
    if (f.output_dtype(i) == pjrt::DType::Float32 &&
        any_nonfinite<float>(arena, numel)) {
      return true;
    }
  }
  return false;
...
void prepare(pjrt::Function& f, Scenario scenario) {
  switch (scenario) {
    ...
    case Scenario::NonfiniteOutput:
      ...
      // Solved well-conditioned first, so the outputs reported on are the
      // singular solve's and not the warm-up's.
...
void provoke(pjrt::Function& f, Scenario scenario) {
  switch (scenario) {
    ...
    case Scenario::DtypeMismatch:
      // Taken but never written through: a write would corrupt the arena on
      // the run where the accessor does not object.
    ...
    case Scenario::NonfiniteOutput:
      // Finite inputs, non-finite arithmetic: reaches the audit after the call.
      f.call();
      break;
...
/// How many times the intruder entered `call()` while the holder was inside
/// one, and what came back on the last of them.
struct RaceResult {
  int attempts = 0;
  std::exception_ptr caught;
};

/// Spin rather than sleep: a sleep returns whenever the scheduler gets round
/// to it, which is precisely the timing this needs to control.
...
/// The fastest of three, so that a scheduling hiccup does not stretch the
/// intruder's delay past the call it is aiming at.
 ...
 *
 * The intruder waits for the holder to enter a call, spins for half a call,
 * then enters itself.  Half a call in is where re-entrancy is unambiguous
 * rather than a coin flip on thread start-up, and it staggers the two calls'
 * output collection -- the microseconds where both threads would write and
 * destroy the same `output_buffers_` list -- so that with the guard off one
 * overlap demonstrates the corruption without a lottery over a double free.
 *
 * With the guard on, "did it fire" is observable, so a lost race is retried.
 * With it off there is nothing to observe but silence, so the first entry
 * made while the holder was inside a call is the whole story.
 *
...
int main(int argc, char** argv) {
  ...
  const std::optional<Scenario> parsed = parse_scenario(scenario_name);
  if (!parsed) {
  ...
  try {
    ...
    options.debug = debug;
    // The re-entrancy guard is a `debug` check only.  With the value audit on,
    // the holder thread would throw about its own outputs, and that exception
    // would be reported as though it were the guard.
    options.check_values = debug && scenario != Scenario::Reentrant;
    pjrt::Function f(runtime, base, options);

    prepare(f, scenario);

    // Outside the reporting block: a failure of the *race* is a failure of
    ...
    switch (scenario) {
      ...
      case Scenario::Reentrant:
        ...
        break;
      // Named rather than defaulted, so a new scenario is a warning here.
      case Scenario::OobInput:
      case Scenario::OobOutput:
      case Scenario::DtypeMismatch:
        break;
    ...
    // Whatever the holder thread stored; leaving main() with it would abort.
